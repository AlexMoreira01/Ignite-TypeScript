...
let CreateCarSpecificationUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!carExists) {
      throw new _AppError.AppError("Car does not exitst!");
    } // Buscas das especificações pelo id -- array de objetos do findByIds


    const specifications = await this.specificationsRepository.findByIds(specifications_id); // Alterar o valor de specification dentro da tabela do carro encontrado, passando os ids das specificaçõe encotradas

    carExists.specifications = specifications; // Update no parametro de carros
