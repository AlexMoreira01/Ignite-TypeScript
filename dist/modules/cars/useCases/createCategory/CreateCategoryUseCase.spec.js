...
describe("Create Category", () => {
  ...
  it("should be able to create a new category", async () => {
    ...
    const categoryCreated = await categoriesRepositoryInMemory.findByName(category.name); // experado que tenha a propriedade
  ...
  it("should not be able to create a new category with name exists", async () => {
    ...
    await expect(createCategoryUseCase.execute({
      name: category.name,
      description: category.description
    })).rejects.toEqual(new _AppError.AppError("Category already exists!")); // Espera que se ele for rejeitado ele seja de uma instancia do appError
  });
});